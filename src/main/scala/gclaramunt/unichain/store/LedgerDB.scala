...
trait LedgerDB[F[_]: MonadCancelThrow]:
  ...
  def addBlockWithTransactions(b: Block, txs: Seq[Transaction]): F[Unit]
...
class LedgerDBImpl[F[_]: MonadCancelThrow](xa: Transactor[F]) extends LedgerDB[F]:
  ...
  private def insertBlock(b: Block): ConnectionIO[Int] =
  ...
  private def insertTransaction(blockId: Long, tx: Transaction): ConnectionIO[Int] =
  ...
  def addBlock(b: Block): F[Int] =
    insertBlock(b).transact(xa)

  def addTransaction(blockId: Long, tx: Transaction): F[Int] =
    insertTransaction(blockId, tx).transact(xa)

  def addBlockWithTransactions(b: Block, txs: Seq[Transaction]): F[Unit] =
    val ops = for
      _ <- insertBlock(b)
      _ <- txs.traverse_(tx => insertTransaction(b.id, tx))
    yield ()
    ops.transact(xa)
